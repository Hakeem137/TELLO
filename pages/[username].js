...
return (
    <div style={{maxWidth:600, margin:'40px auto', padding:20}}>
      ...
      <textarea value={text} onChange={e=>setText(e.target.value)} placeholder="Write your message..." rows={6} style={{width:'100%',padding:12}}/>
...
export async function getServerSideProps({ params }) {
  ...
  if (users.empty) return { props: { user: null, siteKey: process.env.RECAPTCHA_SITEKEY } }

  const doc = users.docs[0].data();
  return { props: { user: { username: doc.username, name: doc.name || null, avatar_url: doc.avatar_url || null }, siteKey: process.env.RECAPTCHA_SITEKEY } };
}
